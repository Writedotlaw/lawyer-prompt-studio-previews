...
config={'name':'AI Sanctions Explorer','organization':'Write.law','intro':'A guide to who appears in the reported decisions, how the errors arose, and what courts did in response. The findings below bring the evidence together for teaching and discussion.','accent':'#217963','study':True,'practice':True,'theme':'paper'}
...
html=html.replace('/*__STYLES__*/',(ROOT/'src/styles.css').read_text()).replace('/*__CORE__*/',(ROOT/'src/core.js').read_text()).replace('/*__CONTENT__*/',(ROOT/'src/content.js').read_text()+'\n'+(ROOT/'src/findings-data.js').read_text()).replace('/*__APP__*/',(ROOT/'src/app.js').read_text().replace('/*__FINDINGS__*/',(ROOT/'src/findings.js').read_text())).replace('__DATA__',j(data)).replace('__CONFIG__',j(config))
